...
minerID_array=[
    'MOID001','MOID002','MOID003','MOID004','MOID005']
...
def start():
    ...
    if request.method=='POST':
        if request.form['submit']=='mine':
            return redirect(url_for('initial1'))
...
def initial1():
    ...
    if request.method=='POST':
        ...
        if user in minerID_array and request.form['submit']=='seechain':
            return redirect(url_for('full_chain'))
       # if user in minerID_array and request.form['submit']=='vote':
       #     return '<body style="color:white;background-color:black;position:middle;"><form style="padding:200px" method="POST"><center><h2>(MINER WITH MINER ID(MOIDXXXX) CANNOT VOTE)</h2><br><center><input type="button" value="Back" style="padding: 10px;" onclick="history.back()"></form><center></body>'
       # if request.form['submit']=='vote':
       #     return redirect(url_for('initial'))
        else:
            return redirect(url_for("control",User="Miner",ID=user))
...
def mine():
    ...
    #response=full_chain()
    return '<body style="color:white;background-color:black;font-family:Georgia, Times New Roman, Times, serif;"><center><form><h1 style="padding:200px;">New Block Created</h1><br><input type="button" value="Back" style="padding : 8px;font-family:Georgia, Times New Roman, Times, serif;font-weight: bold;" onclick="history.back()"></form></center></body>'
    #return response
...
def vote_count():
    print(blockchain.count_bsp(blockchain.chain))
